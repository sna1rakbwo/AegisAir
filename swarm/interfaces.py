...
# ---------------------------------------------------------------------------
# 7. Execution-model assurance decision (C2-prime)
# ---------------------------------------------------------------------------


ExecutionAssuranceReason = Literal[
    "NO_HISTORY",
    "MODEL_CONFORMANT",
    "VELOCITY_RESIDUAL",
    "POSITION_RESIDUAL",
...
class ExecutionResidualRecord(_Frozen):
    """One UAV's one-step execution-model residual."""
...
class RecoverabilityPairRecord(_Frozen):
    """Frozen pairwise stopping-distance reserve reported by C2 predictive gate."""

    drones: tuple[int, int]
    separation_m: float = Field(ge=0.0)
    closing_speed_mps: float = Field(ge=0.0)
    required_distance_m: float = Field(ge=0.0)
    margin_m: float


class ExecutionAssuranceDecision(_Frozen):
    """Frozen C2-prime monitor/switch decision for one control cycle."""

    schema_version: Literal[1] = 1
    active: bool = Field(description="Whether deterministic backup owns control")
    reasons: list[ExecutionAssuranceReason] = Field(min_length=1)
    residuals: dict[int, ExecutionResidualRecord] = Field(default_factory=dict)
    recoverability: list[RecoverabilityPairRecord] = Field(default_factory=list)
    backup_drones: list[int] = Field(default_factory=list)
    qp_feasible: bool
    solve_elapsed_s: float = Field(ge=0.0)
    telemetry_ages_s: dict[int, float] = Field(default_factory=dict)
    consecutive_bad: int = Field(ge=0)
    consecutive_clean: int = Field(ge=0)
    timestamp_ms: int = Field(ge=0)


# ---------------------------------------------------------------------------
# 8. C3 conflict-admission coordination decision
# ---------------------------------------------------------------------------
...
class CoordinationAdmissionDecision(_Frozen):
    """Frozen, auditable C3-Admission decision for one control cycle.

    The decision changes only mission-level passage order and hold goals.  It
    never contains a direct actuator command and therefore cannot bypass RA.
    """

    schema_version: Literal[1] = 1
    step: int = Field(ge=0)
    coordination_mode: CoordinationMode
    admission_triggers: list[AdmissionTrigger] = Field(default_factory=list)
    authorized_drone_ids: list[int] = Field(default_factory=list)
    held_drone_ids: list[int] = Field(default_factory=list)
    frozen_hold_goals: dict[int, Vec3] = Field(default_factory=dict)
    hold_enter_steps: dict[int, int] = Field(default_factory=dict)
    conflict_zone_id: str | None = None
    ra_vetoed: bool = False
    authority_source: Literal["c3_rule", "fallback"]
    timestamp_ms: int = Field(ge=0)


# ---------------------------------------------------------------------------
# 9. C3 terminal-occupancy-aware reservation decision
# ---------------------------------------------------------------------------


ReservationMode = Literal[
    "normal",
    "admission_pending",
    "staging",
    "pass",
...
class CoordinationReservationDecision(_Frozen):
    """Frozen C3-Reservation decision; actuator authority remains with RA."""

    schema_version: Literal[1] = 1
    step: int = Field(ge=0)
    coordination_mode: ReservationMode
    admission_triggers: list[AdmissionTrigger] = Field(default_factory=list)
    authorized_drone_ids: list[int] = Field(default_factory=list)
    held_drone_ids: list[int] = Field(default_factory=list)
    frozen_hold_goals: dict[int, Vec3] = Field(default_factory=dict)
    clearance_drone_id: int | None = Field(default=None, ge=0)
    clearance_goal: Vec3 | None = None
    service_latched_drone_ids: list[int] = Field(default_factory=list)
    cleared_drone_ids: list[int] = Field(default_factory=list)
    final_returned_drone_ids: list[int] = Field(default_factory=list)
    conflict_zone_id: str | None = None
    ra_vetoed: bool = False
    ra_veto_streak: int = Field(default=0, ge=0)
    authority_source: Literal["c3_rule", "fallback"]
    timestamp_ms: int = Field(ge=0)


# ---------------------------------------------------------------------------
# 10. C3 space-time reservation decision
# ---------------------------------------------------------------------------


SpaceTimeReservationMode = Literal[
    "normal",
    "admission_pending",
    "staging",
    "pass",
...
SpaceTimeReservationPhase = Literal["service", "final_return"]
SpaceTimeReservationStatus = Literal[
    "scheduled",
    "open",
    "delayed",
    "completed",
...
class SpaceTimeReservationWindow(_Frozen):
    """One auditable mission-layer time window; it is not an actuator command."""

    slot_id: str = Field(min_length=1)
    phase: SpaceTimeReservationPhase
    drone_ids: list[int] = Field(min_length=1)
    planned_open_step: int = Field(ge=0)
    planned_close_step: int = Field(ge=0)
    actual_open_step: int | None = Field(default=None, ge=0)
    status: SpaceTimeReservationStatus
    delay_steps: int = Field(default=0, ge=0)


class CoordinationSpaceTimeReservationDecision(_Frozen):
    """Frozen C3-STR schedule decision; Runtime Assurance remains final authority."""

    schema_version: Literal[1] = 1
    step: int = Field(ge=0)
    coordination_mode: SpaceTimeReservationMode
    admission_triggers: list[AdmissionTrigger] = Field(default_factory=list)
    authorized_drone_ids: list[int] = Field(default_factory=list)
    held_drone_ids: list[int] = Field(default_factory=list)
    frozen_hold_goals: dict[int, Vec3] = Field(default_factory=dict)
    committed_waypoints: dict[int, list[Vec3]] = Field(default_factory=dict)
    reservation_windows: list[SpaceTimeReservationWindow] = Field(default_factory=list)
    ...
    total_delay_steps: int = Field(default=0, ge=0)
    authority_source: Literal["c3_rule", "fallback"]
    timestamp_ms: int = Field(ge=0)


# ---------------------------------------------------------------------------
# 11. Append-only log event
# ---------------------------------------------------------------------------
