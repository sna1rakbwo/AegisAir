...
> ⚠️ **参数说明（2026-08-21）**：本结果的 `tau_px4=0.2 s` 是 RA 一步 sampled-data
> barrier 的短时速度响应时间常数，不是 Phase 7 阶跃识别拟合的完整沉降 `τ_hat=0.7 s`
> （经验区间 `[0.53, 1.76] s`）。二者不是同一个量：0.7 s 是速度收敛到稳态的完整时间，
> 一步 barrier 只需短时响应。实测把 `tau_px4` 改到 0.7 会让一步 β 系数过小、刹车失效、
> 真实 PX4 里直接逼近碰撞（min_distance 0.25–0.6 m），而 0.2 保持 ~1.2 m。故
> `tau_px4=0.2` 保留，不做 0.7 替换。
