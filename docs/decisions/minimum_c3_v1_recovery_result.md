...
> ⚠️ **参数勘误（2026-08-21）**：本结果使用的 `tau_ctrl=tau_px4=0.2 s` 是错误的。
> Phase 7 阶跃识别实测的 velocity-tracking tau 是 `τ_hat=0.7 s`（经验区间
> `[0.53, 1.76] s`），`tau_ctrl < 0.1 s`。正确值应为 `tau_px4=0.7`、`tau_ctrl=0.1`。
> 本文保留为历史记录，若需用于最终论文须按正确 tau 重跑。
